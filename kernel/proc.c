...
// Type 1 for lottery scheduling 0/default for Round Robin Scheduling
    ...
    case 1:
            for (;;) 
            {
                ...
                for (p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
                    if (p->state != RUNNABLE)
                        continue;
                    sumtickets += p->priority;
                }
                luckyNumber = prng_limit(sumtickets - 1);
                for (p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
                    if (p->state != RUNNABLE)
                        continue;
                    ...
                    if (counter > luckyNumber) {
                        if (previousPid != p->pid) {
                            printProcessTable();
                            cprintf("Total number of tickets: %d Winning Number: %d Winning PID: "
                                    "%d\n",
                                    sumtickets, luckyNumber, p->pid);
                        }
                        foundproc = 1;
                        proc = p;
                        switchuvm(p);
